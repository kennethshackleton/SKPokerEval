...
FiveEval::FiveEval() {
  ...
  for (int i = 0; i < NUMBER_OF_FACES; ++i) {
    for (int j = 2; j < NUMBER_OF_FACES; ++j) {
      for (int k = 1; k < j; ++k) {
        for (int l = 0; l < k; ++l) {
          if (i != j && i != k && i != l) {
            mRankArray[(2*face[i]) + face[j] + face[k] + face[l]] = n++;
          }
        }
      }
    }
  ...
  for (int i = 1; i < NUMBER_OF_FACES; ++i) {
    for (int j = 0; j < i; ++j) {
      for (int k = 0; k < NUMBER_OF_FACES; ++k) {
        // No full-houses.
        if (k != i && k != j) {
          mRankArray[(2*face[i]) + (2*face[j]) + face[k]] = n++;
        }
      }
    }
  }
  ...
  for (int i = 0; i < NUMBER_OF_FACES; ++i) {
    for (int j = 1; j < NUMBER_OF_FACES; ++j) {
      for (int k = 0; k < j; ++k) {
        // No quads.
        if (i != j && i != k) {
          mRankArray[(3*face[i]) + face[j] + face[k]] = n++;
        }
      }
    }
  }
  ...
  mRankArray[face[12] + face[0] + face[1] + face[2] + face[3]] = n++;

  // Usual straight non-flush.
  for (int i = 0; i < 9; ++i) {
    mRankArray[face[i] + face[i+1] + face[i+2] + face[i+3] + face[i+4]] = n++;
  }
  ...
  for (int i = 0; i < NUMBER_OF_FACES; ++i) {
    for (int j = 0; j < NUMBER_OF_FACES; ++j) {
      if (i != j) {
        mRankArray[(3 * face[i]) + (2*face[j])] = n++;
      }
    }
  }
  ...
  for (int i = 0; i < NUMBER_OF_FACES; ++i) {
    for (int j = 0; j < NUMBER_OF_FACES; ++j) {
      if (i != j) {
        mRankArray[(4*face[i]) + face[j]] = n++;
      }
    }
  }

  // Low straight flush.
  mFlushRankArray[face_flush[0] + face_flush[1] + face_flush[2] +
      face_flush[3] + face_flush[12]] = n++;

  // Usual straight flush.
  for (int i = 0; i < 9; ++i) {
    mFlushRankArray[face_flush[i] + face_flush[i+1] + face_flush[i+2] +
...
short unsigned FiveEval::GetRank(int const card_one, int const card_two,
    int const card_three, int const card_four, int const card_five) const {
...
short unsigned FiveEval::GetRank(int const card_one, const int card_two,
    const int card_three, const int card_four, const int card_five,
    const int card_six, const int card_seven) const {
