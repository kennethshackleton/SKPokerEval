...
// Key values are 32-bit integers, the first 9 bits
// are given to the suit and all of the remaining
// 23 bits are given to the face value. We add card
// keys together to generate a hand key, check the
// first 9 bits to see if it's a flush. If it's not
// a flush we bit shift >> 9 the hand key and look-up
// the non-flush rank at the corresponding index.
// We really do need all of the 32 bits on offer.
...
{		
	...
	for (int i = 0; i < MAX_SEVEN_FLUSH_KEY_INT + 1; i++)
  {
	...
	FiveEval * fiveCardEvaluator = [FiveEval alloc];
  fiveCardEvaluator = [fiveCardEvaluator init];
            ...
            for(int n = 0; n <= m; n++)
            {
              for(int p = 0; p <= n; p++)
              {
                if(i != m && j != n && k != p)
                {
                  ...
                  short rank = [fiveCardEvaluator getRankOfSeven:4*i
                                                                :4*j
                                                                :4*k
                                                                :4*l
                                                                :4*m+1
                  ...
                  key = (key < CIRCUMFERENCE_SEVEN ? key :
      ...
      for (int k = 2; k < j; k++)
      {
        for (int l = 1; l < k; l++)
        {
          for (int m = 0; m < l; m++)
          {
            ...
            short rank = [fiveCardEvaluator getRankOfFive:4*i
                                                         :4*j
                                                         :4*k
                                                         :4*l
                                                         :4*m];
                        ...
                        :(const int) card_7
{
	...
	if (FLUSH_SUIT == NOT_A_FLUSH)
  {
  	KEY = (KEY >> NON_FLUSH_BIT_SHIFT);
		...
		return (KEY < CIRCUMFERENCE_SEVEN ?
            rankArray[KEY] :
	...
	else
  {
      //** This could be a good place to optimize **
		int FLUSH_KEY = (deckcardsSuit[card_1] == FLUSH_SUIT ?
                     deckcardsFlush[card_1] : 0) +
                    (deckcardsSuit[card_2] == FLUSH_SUIT ?
                     deckcardsFlush[card_2] : 0) +
                    (deckcardsSuit[card_3] == FLUSH_SUIT ?
                     deckcardsFlush[card_3] : 0) +
                    (deckcardsSuit[card_4] == FLUSH_SUIT ?
                     deckcardsFlush[card_4] : 0) +
                    (deckcardsSuit[card_5] == FLUSH_SUIT ?
                     deckcardsFlush[card_5] : 0) +
                    (deckcardsSuit[card_6] == FLUSH_SUIT ?
                     deckcardsFlush[card_6] : 0) +
                    (deckcardsSuit[card_7] == FLUSH_SUIT ?
                     deckcardsFlush[card_7] : 0);
...
-(NSString *) computePreFlopEquityForSpecificHoleCards:(int *) hole_cards
               withNumberOfPlayers:(int) number_of_players
{
	...
	for(int p = 0; p < number_of_players; p++)
  {
		...
		player_suit_stem[p] = deckcardsSuit[hole_cards[P]] +
                          deckcardsSuit[hole_cards[P + 1]];
  }
	////////
	...
	do
  {
		...
		do
    {
			...
			unsigned int table_key_cumulative_to_2 = deckcardsKey[table_card_1] +
			...
			do
      {
				...
				unsigned int table_key_cumulative_to_3 = table_key_cumulative_to_2 +
				...
				do
        {
					...
					unsigned int table_key_cumulative_to_4 = table_key_cumulative_to_3 +
					...
					do
          {
						...
						unsigned int table_key = table_key_cumulative_to_4 +
						...
						else
            {
               ...
               table_flush_key = (deckcardsSuit[table_card_1] == FLUSH_SUIT ?
                                 ...
                                 (deckcardsSuit[table_card_5] == FLUSH_SUIT ?
                                  deckcardsFlush[table_card_5] : 0) : NO );
						...
						do
            {
							...
							else if (current_best_rank == next_rank)
              {
						...
						int incremental_equity =
                    equity_share[number_of_players_sharing_the_pot];
...
- (NSString *) computeFlopEquityForSpecificHoleCards:(int *) hole_cards
                withTableCards:(int *) table_cards
                andNumberOfPlayers:(int) number_of_players
{
	// Count and list the undealt cards, ie. the remaining deck.
	int undealtCards[DECK_SIZE];
	int number_of_undealt_cards = 0;
...
- (NSString *) computeTurnEquityForSpecificHoleCards:(int *) hole_cards
                withTableCards:(int *) table_cards
                andNumberOfPlayers:(int) number_of_players
{
	//Count and list the undealt cards, ie. the remaining deck
...
- (NSString *) computeRiverEquityForSpecificHoleCards:(int *) hole_cards
                withTableCards:(int *) table_cards
                andNumberOfPlayers:(int) number_of_players
{
	//Cumulative equities
	...
	for(int i = 0; i < MAX_NUMBER_OF_PLAYERS; i++)
  {
	...
	current_best_rank = [self getRankOfSeven:card_1
                                          :card_2
                                          :table_cards[0]
                                          :table_cards[1]
                                          :table_cards[2]
	...
	for(int i = 1; i < number_of_players; i++)
  {
		...
		next_rank = [self getRankOfSeven:card_1
                                    :card_2
                                    :table_cards[0]
                                    :table_cards[1]
                                    :table_cards[2]
